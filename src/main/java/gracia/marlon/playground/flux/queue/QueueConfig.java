...
public class QueueConfig {
	...
	@Bean
	@ConditionalOnProperty(prefix = "spring.cloud.aws.sqs", name = "enabled", havingValue = "true")
	QueueConsumer queueConsumer(SqsAsyncClient sqsClient, LoadMoviesService loadMoviesService, Environment env) {
		return new QueueConsumer(sqsClient, loadMoviesService, env);
	}

	@Bean
	@ConditionalOnProperty(prefix = "spring.cloud.aws.sqs", name = "enabled", havingValue = "true")
	QueuePublisherService queuePublisherServiceSQS(SqsAsyncClient sqsClient) {
		return new QueuePublisherServiceImpl(sqsClient);
	}
